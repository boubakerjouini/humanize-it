...
function PlanBadge({ plan }: { plan: string }) {
  ...
  if (isTeam) return (
    <div style={{
      ...
      <Crown size={10} color="#7e22ce" />
      <span style={{ fontSize: "10px", fontWeight: 700, color: "#7e22ce", letterSpacing: "0.5px" }}>TEAM</span>
    </div>
  );
  ...
  if (isPro) return (
    <div style={{
      ...
      <span style={{ fontSize: "10px", fontWeight: 700, color: "#7e22ce", letterSpacing: "0.5px" }}>PRO</span>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: "10px", fontWeight: 600, color: "#9ca3af", letterSpacing: "0.5px" }}>FREE</span>
    </div>
  );
}
    ...
    <div style={{ display: "flex", height: "100dvh", overflow: "hidden", background: "#ffffff" }}>
        ...
        <Link href="/" style={{
          ...
          <div>
            <div style={{ fontSize: "14px", fontWeight: 700, lineHeight: 1.2, fontFamily: "var(--font-heading)" }}>
              <span style={{ color: "#3b0764" }}>Humanize</span><span style={{ color: "#9333ea" }}>It</span>
            </div>
            <div style={{ fontSize: "10px", color: "#9ca3af", marginTop: "1px" }}>Writing assistant</div>
          </div>
        </Link>

        {/* Nav */}
        <nav style={{ flex: 1, padding: "16px 10px", display: "flex", flexDirection: "column", gap: "2px" }}>
          <div style={{ fontSize: "10px", color: "#9ca3af", letterSpacing: "0.8px", padding: "0 10px 8px", textTransform: "uppercase", fontWeight: 600 }}>
            Workspace
          </div>
          {NAV_ITEMS.map(({ href, label, icon: Icon, desc, exact }) => {
            ...
            return (
              <Link key={href} href={href} style={{
                ...
                <Icon size={15} color={active ? "#7e22ce" : "#9ca3af"} />
                <div>
                  <div style={{ fontSize: "13px", lineHeight: 1.2 }}>{label}</div>
                  {active && <div style={{ fontSize: "10px", color: "#a855f7", marginTop: "1px" }}>{desc}</div>}
                </div>
              </Link>
            );
          })}
      ...
      <nav className="flex md:hidden" style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "4px", flex: 1, padding: "8px 0" }}>
          ...
          <span style={{ fontSize: "10px", color: "#9ca3af" }}>Account</span>
        </div>
      </nav>
    </div>
  );
