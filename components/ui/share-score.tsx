...
export function ShareScore({ score, humanizedScore }: ShareScoreProps) {
  ...
  const twitterText = encodeURIComponent(
    `I just humanized my AI text to ${Math.round(humanizedScore)}% human with HumanizeIt! Try it free: https://humanizeit.app`
  );
  const linkedinUrl = encodeURIComponent("https://humanizeit.app");
  const linkedinTitle = encodeURIComponent(
    `I just humanized my AI text from ${Math.round(score)}% AI to ${Math.round(humanizedScore)}% with HumanizeIt!`
  );
